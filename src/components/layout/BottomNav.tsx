...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, onStartJourneyClick }) => {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 glass-panel border-t border-slate-800/90 px-1 py-1.5 bg-slate-950/95 backdrop-blur-xl">
      <div className="max-w-md mx-auto flex items-center justify-around relative">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 py-1 px-1.5 rounded-xl transition-all cursor-pointer ${
            activeTab === 'reels' ? 'text-purple-400 bg-purple-500/10 font-bold' : 'text-purple-400/80 hover:text-purple-300'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 py-1 px-1.5 rounded-xl transition-all cursor-pointer ${
            activeTab === 'gov-hub' ? 'text-emerald-400 bg-emerald-500/10 font-bold' : 'text-emerald-400/80 hover:text-emerald-300'
          }`}
        >
          <Building2 className="w-4 h-4" />
          <span className="text-[9px]">Gov Hub</span>
        </button>

        {/* Central Floating "GO" Action Button */}
        <div className="relative -top-3 shrink-0">
          <button
            ...
            className="w-11 h-11 rounded-full bg-gradient-to-tr from-sky-500 via-indigo-500 to-emerald-400 p-0.5 shadow-xl shadow-sky-500/40 flex items-center justify-center transform active:scale-95 transition-transform cursor-pointer group"
            title="Start Journey"
          >
